...
public class FirstCartFragment extends Fragment implements CartAdapter.DeleteItem, CartAdapter.TotalPrice {
    ...
    @Override
    public void onDeleteResult(Items item) {
        Utils.deleteItemFromCart(getActivity(), item);
        ArrayList<Items> cartItems = Utils.getCartItems(getActivity());
        if (null != cartItems) {
            if (cartItems.size()>0) {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.activity_first_cart_fragment, container, false);
